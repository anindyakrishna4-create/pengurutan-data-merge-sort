...
if st.button("Mulai Simulasi Merge Sort"):
    ...
    sorted_data, history = merge_sort(data_list)
    ...
    for step, state in enumerate(history):
        ...
        chart = alt.Chart(df_vis).mark_bar().encode(
            ...
            color=alt.Color('Tipe', 
                            scale=alt.Scale(domain=['Pivot/Key Kiri', 'Pivot/Key Kanan', 'Rentang Merge Selesai', 'Normal'], 
                                            range=['#6AA84F', '#F1C232', '#4A86E8', '#999999']), # Hijau: Kiri, Kuning: Kanan, Biru: Selesai
                            legend=None),
